...
this.overviewMenu = function(finalMenu, totalPrice, numGuest){
		$("#lastOverview").empty();
		$("#lastOverview").show();
		$("#lastOverview").append('<div id="overview" class="col-xs-12 col-sm-12 col-md-12"><div id="mainContent" class="col-md-6"><h1 id="headerContent">My dinner: '+numGuest+' people</h1></div><button id="back2" type="button" class="btn btn-warning btn-responsive back">Go back and edit dinner</button></div>');
		var count = 0;
	    var str = "";
		...
		for(var x in finalMenu){
			...
			str += '<div class="col-xs-3 col-sm-3 col-md-3 " style="vertical-align: top;text-align: center;margin: 0 auto;"><div style="margin:0 auto;width: 180px;"><div style="background-color:grey;display: table;margin:0 auto;"><div style="overflow: hidden;width: 180px;"><image class="img-thumbnail" id="imgOverview" src="images/' + finalMenu[x].image + '"></image></div><h3 class="" style="">' + finalMenu[x].name + '</h3></div><p style="text-align: left;width: 180px;">' + finalMenu[x].description + '</p></div></div>';
		}
